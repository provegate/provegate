...
| FR-3                  | `pnpm --filter provegate test test/verify-test-inputs.test.ts`            | pkg      | passed  | 24/24 incl. the review round-1..4 closures | ten deny causes by name; concatenation non-catch documented; positive control                      |
...
| 2026-07-29 | 5.x     | four surfaces one move; ADR row hand-placed (pre-existing ADR prettier-warn confirmed on main, untouched); harness grown to 24/24 across the review rounds                                                                                                        |
