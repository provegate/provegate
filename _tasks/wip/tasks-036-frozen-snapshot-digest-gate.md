...
| FR-2                  | `pnpm test`                                                               | repo     | passed  | 1368 tests, 57 files (post-review; ledger updated per round)                            | migrated suite green through turbo; zero behavior change                                           |
| FR-3                  | `pnpm --filter provegate test test/verify-test-inputs.test.ts`            | pkg      | passed  | 22/22 incl. the review round-1/2/3 closures | ten deny causes by name; concatenation non-catch documented; positive control                      |
...
| 2026-07-29 | 5.x     | four surfaces one move; ADR row hand-placed (pre-existing ADR prettier-warn confirmed on main, untouched); harness grown to 22/22 across the review rounds                                                                                                        |
