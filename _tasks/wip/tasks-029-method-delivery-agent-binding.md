...
| ---- | --------- | ----- | --------------------------------------------------------------------------------------------------------------------------- | ------- | ------------------------------------------------------------------------------------------------------------- |
| 5.6  | manual-qa | owner | The generated `.cursor/rules/prd-workflow.mdc` actually attaches in a real Cursor session on a `_prds/**` file                | **verified 2026-07-27** | Live Cursor, this repository. Attached on `_prds/wip/prd-029-*.md`; **did NOT attach on `README.md`** while `brain.mdc` (`alwaysApply: true`) did. The negative half is the real evidence — it proves the `globs` scope, not merely that a rule loads. Mechanically pre-checked: frontmatter structurally identical to `source-snapshot/rules/prd-workflow.mdc`, and the `globs` line byte-identical to it |
| 5.6  | manual-qa | owner | A generated `.claude/commands/prd-*.md` is invocable in a real Claude Code session and loads the store protocol it points at | **verified 2026-07-27** | Live Claude Code, this repository. `/prd-3` was discovered, invoked, and its directive followed to `.provegate/prompts/phase-3-task-generator.md`, whose STOP-until-Go rule reached the session's context — the exact rule whose absence opened this work item. **The live test also found a defect the grammar test could not**: Claude Code takes a command's FIRST LINE as its listing description, so the banner renders as `prd-3: <!-- GENERATED by provegate 0.2.0…`. Recorded as a deferral; the row's required check passed |
