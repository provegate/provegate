...
- [ ] 0.0 Pre-flight
  - [ ] 0.1 Open each Memory Context record under `_brain/learnings/` and confirm the
        paths and commands it names still exist; record any stale finding in
        **Deferrals & Decisions**.
  - [ ] 0.2 Re-run `node packages/provegate/dist/cli.js queue` and confirm no active
  ...
  - [ ] 0.3 Claim the work item: `gate open PRD-025 --worktree` from a clean base
        (worktree refuses uncommitted control files — commit first). Confirm the lease
        exists under `_state/locks/` on the MAIN checkout.
  - [ ] 0.4 In the worktree: `pnpm install`, then baseline `pnpm --filter provegate test`
        green before any edit; record the test count.
  - [ ] 0.5 Re-measure the two dated assumptions the grammar was written against:
        ...
        adjustment.
- [ ] 1.0 Config surface — the three `wiring.*` keys and the containment export (FR-2)
  - [ ] 1.1 `packages/provegate/src/core/config/types.ts`: add `wiring.scriptsDir`,
  ...
  - [ ] 1.2 `packages/provegate/src/core/config/defaults.ts`: defaults `scripts/verify`,
        `.githooks`, `scripts/verify/verify-workflow.mjs`.
  - [ ] 1.3 `packages/provegate/src/core/config/validate.ts`: semantic validation —
  ...
  - [ ] 1.4 `packages/provegate/src/core/config/load.ts`: add `export` to
  ...
  - [ ] 1.5 New `packages/provegate/test/config-wiring.test.ts`: the three defaults
        resolve; absolute / escaping / non-string each rejected with a named issue;
        absent directory accepted.
  - [ ] 1.6 Same file: three symlink-escape fixtures — each configured read path
...
- [ ] 2.0 FR-3 machinery — one scanner pass, one command shape, the bundle grammar
  - [ ] 2.1 `packages/provegate/src/core/gates/wiring.ts`: implement the scanner as a
  ...
  - [ ] 2.2 Implement the command shape: strip `NAME=value` wrappers and leading `env`
  ...
  - [ ] 2.3 Implement FR-3(a) key derivation: for each registered script matching
  ...
  - [ ] 2.4 Implement the bundle reader per FR-3(c): opening line = column-zero
  ...
  - [ ] 2.5 Add the surface-count report to the audit's output (how many surfaces were
        actually read) — the visibility requirement that makes a silently-lost surface a
        number a maintainer can watch.
  - [ ] 2.6 Tests (`wiring.test.ts`): the five scanner deny rows with paired positive
  ...
  - [ ] 2.7 Tests: command-shape rows with paired controls — head token (echo body vs
        ...
        node head yes).
  - [ ] 2.8 Tests: the preload row's stated BEHAVIOR REVERSAL —
  ...
  - [ ] 2.9 Tests: the four bundle rows with controls — column anchor (indented sole
  ...
  - [ ] 2.10 W1 adversarial comment fixtures: `#` mid-token (`a#b` ordinary), `#` inside
  ...
  - [ ] 2.11 W3: verify `deno <path>` (bare, no `run`) against deno's current CLI
...
- [ ] 3.0 FR-2 — the three surfaces, read through the new machinery
  - [ ] 3.1 `wiring.ts`: read the configured hooks directory (every file's body is a
        ...
        symlink escape.
  - [ ] 3.2 `wiring.ts`: read the configured bundle path through the 2.4 reader; bundle
        membership (bare basename in the declared list) counts as wiring; absent or
        unparseable = not a surface.
  - [ ] 3.3 `wiring.ts`: read every `package.json` script body whose name does NOT match
  ...
  - [ ] 3.4 Keep the CI reading at `run:` text exactly as today — the deliberate
        narrowing stays stated, not reconciled.
  - [ ] 3.5 Extend the "wired nowhere" message's parenthetical (`wiring.ts:245-247`
        today) to name the three new surfaces it searched.
  - [ ] 3.6 Tests: a check wired ONLY through a hook registers as wired; only through
        ...
        with their controls.
- [ ] 4.0 FR-1 — the on-disk → registered direction
  - [ ] 4.1 `wiring.ts`: selection — candidates are files under `wiring.scriptsDir`
        whose basename matches `^verify-.*\.mjs$` (a filename pattern, distinct from
        `verifyScriptPattern`; do not reuse one for the other).
  - [ ] 4.2 `wiring.ts`: registration — a candidate is registered when some
  ...
  - [ ] 4.3 Tests: unregistered on-disk script fails by name with the directory read
...
- [ ] 5.0 FR-4 — the release entry and the re-anchored selector
  - [ ] 5.1 `packages/provegate/test/changeset-entry.test.ts`: give each existing
  ...
  - [ ] 5.2 Add this PRD's `.changeset/` entry: minor bump for `provegate`, naming the
  ...
  - [ ] 5.3 Extend the test with this entry's own assertion group discriminated by the
...
- [ ] 6.0 Migration & Rollback Plan (infra-class explicit parent)
  - [ ] 6.1 Confirm the audit changes are additive end-to-end: with no `wiring` block in
  ...
  - [ ] 6.2 Write the revert path into the implementation commit message trailer and
  ...
  - [ ] 6.3 Assert the post-release rollback rule is recorded where a maintainer will
        ...
        `validate.ts:193`).
  - [ ] 6.4 Run `pnpm verify:gates-wired` — the script this audit will replace must
        still agree with the repository's real wiring while both exist (PRD floor).
- [ ] 7.0 Phase 5 — Testing (every §11 command, no additions, no omissions)
  - [ ] 7.1 `pnpm --filter provegate test test/wiring.test.ts` — FR-1, FR-2 and FR-3
        rows: full deny matrix + paired controls per the §11 FR-3 Notes.
  - [ ] 7.2 `pnpm --filter provegate test test/config-wiring.test.ts` — FR-2 config row.
  - [ ] 7.3 `pnpm --filter provegate test test/changeset-entry.test.ts` — FR-4 row.
  - [ ] 7.4 Cross-cutting floor: `pnpm check-types`, `pnpm lint`, `pnpm test`,
  ...
  - [ ] 7.5 W4 mutation checks: revert the dash rule, the comment rule, and the bundle
  ...
  - [ ] 7.6 Update the Verification Ledger with evidence per row.
...
| ------------------ | ----------------------------------------------------------- | ----- | ------- | -------- | -------------------------- |
| FR-1               | `pnpm --filter provegate test test/wiring.test.ts`          | pkg   | pending |          | on-disk → registered, directory from config |
| FR-2               | `pnpm --filter provegate test test/wiring.test.ts`          | pkg   | pending |          | three surfaces; YAML comment no; verify-prefixed body no |
| FR-2               | `pnpm --filter provegate test test/config-wiring.test.ts`   | pkg   | pending |          | defaults, rejections, absent-dir OK, symlink escapes refused |
| FR-3               | `pnpm --filter provegate test test/wiring.test.ts`          | pkg   | pending |          | full deny matrix + paired controls per §11 Notes |
| FR-4               | `pnpm --filter provegate test test/changeset-entry.test.ts` | pkg   | pending |          | minor entry names the three keys + surfaces; selectors discriminated |
...
| gates-wired        | `pnpm verify:gates-wired`                                   | repo  | pending |          | the replaced script still agrees while both exist |
