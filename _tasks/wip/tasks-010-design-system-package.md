...
- [ ] 1.0 Package scaffold + turbo wiring
  - [ ] 1.1 Create `packages/design/package.json`: name `@provegate/design`,
  ...
  - [ ] 1.2 Add `packages/design/tsconfig.json` and `tsup.config.ts` matching the
  ...
  - [ ] 1.3 Wire `turbo.json`: add a `generate-tokens` task, add
        `packages/design/src/tokens.ts` to `build`'s `inputs`, confirm
        `pnpm-workspace.yaml` `packages/*` glob resolves the package. (FR-8)
  - [ ] 1.4 Confirm `pnpm --filter @provegate/design build` resolves and `pnpm
        build` orders design before dependents. (FR-1, FR-8)

- [ ] 2.0 Token source of truth + typed law
  - [ ] 2.1 Author `packages/design/src/tokens.ts`: warm neutral ramp; proof-green,
  ...
  - [ ] 2.2 Encode the color law and closed ledger vocabulary as union types:
        ...
        type. (FR-7)
  - [ ] 2.3 Export brand asset paths (`logo.svg`, `favicon.svg`) from `./tokens`
        so consumers reference rather than copy. (FR-6)

- [ ] 3.0 Generator + generated outputs + byte-identity gate
  - [ ] 3.1 Write `packages/design/scripts/generate-tokens.ts`: emit
        ...
        source. (FR-3)
  - [ ] 3.2 **W1 — generator vs formatter**: make the generator emit
  ...
  - [ ] 3.3 Pin determinism: fixed key order, fixed number formatting, trailing
        newline — so the byte-identity gate is stable, not flaky. (W1)
  - [ ] 3.4 `test/tokens.test.ts`: regenerate into a temp dir and assert
...
- [ ] 4.0 Ported token layers + self-hosted fonts + assets
  - [ ] 4.1 Port `typography.css`, `spacing.css`, `effects.css` and `styles.css`
        from the handoff verbatim; preserve every `--pg-*` name. (FR-4)
  - [ ] 4.2 **W2 — font provenance**: vendor upstream IBM Plex Sans + Mono woff2
        UNMODIFIED into `assets/fonts/`, commit `OFL.txt`, record source URL +
        checksum (in README). No subsetting toolchain. (FR-5, W2)
  - [ ] 4.3 Rewrite `tokens/fonts.css` as local `@font-face` (relative `src`,
        `font-display: swap`); the Google Fonts `@import` must not survive
        anywhere in the repo. (FR-5)
  - [ ] 4.4 Move `assets/logo.svg` + `assets/favicon.svg` in unchanged. (FR-6)
  - [ ] 4.5 `test/assets.test.ts`: no `http`/`https` URL in any shipped CSS;
...
- [ ] 5.0 CLI string builders (FR-11)
  - [ ] 5.1 Author `src/cli/cards.ts`: pure `handoffCard` + `stopCard` builders
  ...
  - [ ] 5.2 Author `src/cli/status-line.ts`: pure builder for the
        `<glyph> phase N · name · command · exit code · verdict` grammar. (FR-11)
  - [ ] 5.3 `src/cli/index.ts` exports theme + builders as the `./cli` entry; no
        `.css`, no React, no third-party import. (FR-11, FR-9)
  - [ ] 5.4 `test/cards.test.ts`: assert `handoffCard`/`stopCard` output is
        byte-identical to the current `cards.ts` fixtures. (FR-11)

- [ ] 6.0 Invariant tests (import-graph + contrast)
  - [ ] 6.1 **W4** — `test/cli-entry.test.ts`: walk the TRANSITIVE import graph
        from the `./cli` entry; fail on the first `.css`, React, or non-relative
        (third-party) import. (FR-9, W4)
  - [ ] 6.2 Assert `@provegate/design` declares zero runtime `dependencies`
        (read its package.json). (FR-9)
  - [ ] 6.3 **W5** — pure-function contrast test over the semantic token pairs
        (text-on-surface, status-on-terminal-bg) in both themes; assert WCAG AA.
        (W5)

- [ ] 7.0 README durable artifact
  - [ ] 7.1 Write `packages/design/README.md`: the origin rule, the color law
...
- [ ] 8.0 Phase 5 — Testing
  - [ ] 8.1 Run every PRD §11 command; paste command + trimmed output into the
        Verification Ledger. A listed-but-not-run command is never `passed`.
  - [ ] 8.2 Run the cross-cutting floor: `pnpm check-types`, `pnpm lint`,
...
| ------------------ | -------------------------------------------------------------- | ------ | ------- | -------- | --------------------------- |
| FR-1               | `pnpm --filter @provegate/design build`                        | design | pending |          | package builds, exports      |
| FR-2               | `pnpm --filter @provegate/design test test/tokens.test.ts`     | design | pending |          | hexes present, once each     |
| FR-3               | `pnpm --filter @provegate/design test test/tokens.test.ts`     | design | pending |          | regenerate → byte-identical  |
| FR-4               | `grep -c "pg-space-" packages/design/src/tokens/spacing.css`   | design | pending |          | ported layers present        |
| FR-5               | `pnpm --filter @provegate/design test test/assets.test.ts`     | design | pending |          | no http; OFL; src resolves    |
| FR-6               | `test -f packages/design/assets/logo.svg`                      | design | pending |          | brand assets vendored        |
| FR-7               | `pnpm --filter @provegate/design test test/tokens.test.ts`     | design | pending |          | closed verdict set + glyphs  |
| FR-8               | `pnpm build`                                                    | root   | pending |          | turbo orders design first    |
| FR-9               | `pnpm --filter @provegate/design test test/cli-entry.test.ts`  | design | pending |          | no css/react/third-party      |
| FR-10              | `grep -c "GREEN IS EARNED" packages/design/README.md`          | design | pending |          | color law documented         |
| FR-11              | `pnpm --filter @provegate/design test test/cards.test.ts`      | design | pending |          | card text byte-identical     |
...
| gate-check         | `node packages/provegate/dist/cli.js check PRD-010`           | repo   | pending |          | PRD passes its own gate      |
...
| ---- | -------- | ----- | -------------- | ------ | ----- |
| —    | —        | —     | none — every gate is machine-checkable | — | Autonomous Close eligibility candidate; PRD header says operator-gated, revisit at close |
