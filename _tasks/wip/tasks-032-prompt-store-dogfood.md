...
- [ ] 0.0 Pre-flight (W3 re-verified at claim)
  - [ ] 0.1 Fresh `pnpm verify:workflow` on current main — green required; a red
        belongs to whoever owns the drifted surface (§12), STOP and surface.
  - [ ] 0.2 `gate queue` — locks dir empty and no overlapping execution claim
        (the 036 contest resolved at its close; re-establish, don't trust).
  - [ ] 0.3 `node packages/provegate/dist/cli.js open PRD-032 --worktree`; board
...
- [ ] 1.0 FR-7 — the Claude listing line (adapter first, so every later render
      carries it)
  - [ ] 1.1 Test FIRST in `packages/provegate/test/prompts.test.ts`: for every
  ...
  - [ ] 1.2 `packages/provegate/src/core/run/prompts.ts`: the Claude adapter emits
        a phase-descriptive listing line first, banner intact below (attribution
        depends on it — §12). Test green; rebuild the package.
- [ ] 2.0 FR-1 — the owner-answered values (W6)
  - [ ] 2.1 Run the built CLI's `init --prompts` PRINT and diff its key set against
        FR-1's ten-value table: an EXTRA printed key = STOP (owner decision); a
        vanished key = dropped, recorded below.
  - [ ] 2.2 `workflow.config.json`: the `prompts` block — `dir: .provegate`, all
...
- [ ] 3.0 FR-2 + FR-5 — the atomic activation (Migration §, order binding)
  - [ ] 3.1 BEFORE committing: run the render, capture the printed generated set
        (the `generatedPaths()` list — ~30 paths at the current corpus) into the
        Progress Log — it is the rollback's deletion manifest.
  - [ ] 3.2 ONE commit: the filled block, `templates.prd` → the rendered PRD
  ...
  - [ ] 3.3 Confirm live: `node packages/provegate/dist/cli.js check --prompts` —
        every planned path `current`; `pnpm verify:prompts` green (committed store
        equals a fresh render).
- [ ] 4.0 FR-8 — one review-template authority, closed at the source
  - [ ] 4.1 Align BOTH copies to required-Quorum (root
        ...
        root-only).
  - [ ] 4.2 NEW `scripts/verify/verify-review-quorum-authority.mjs`: semantic,
        both copies, loud on a missing file; `package.json` alias
        `verify:review-quorum`.
- [ ] 5.0 FR-3 — the bite, executed (W5)
  - [ ] 5.1 NEW `scripts/verify/verify-prompts-mutation.mjs` + alias
        ...
        of the file).
- [ ] 6.0 FR-9 — the integration set, one gate-green sequence (W1)
  - [ ] 6.1 `verify-workflow.mjs` `CHECKS`: append exactly the filename literal
        `'verify-review-quorum-authority.mjs'` — never the alias; the bundle gains
        exactly ONE member from this PRD (§12).
  - [ ] 6.2 `gates.manifest.json` `wiringExceptions`: the shrink-only
        `verify:prompts-mutation` entry with the Phase-5-evidence justification.
  - [ ] 6.3 `script-classes.json`: both scripts `repo`; ADR-0004 Classification
        table: both rows, HAND-PLACED — no `pnpm format` over the ADR;
        `pnpm verify:brain` + `pnpm verify:script-classes` green after.
  - [ ] 6.4 Pack-drift check: the packed practices template edit (4.1) may trip
        the twin ledger — reconcile with the counterpart decision recorded.
- [ ] 7.0 FR-4 + FR-6 — read-only liveness assertions
  - [ ] 7.1 `pnpm verify:workflow` — the prompts check runs INSIDE the bundle with
        real content; `gate check --wiring` green (registered and executing).
  - [ ] 7.2 `pnpm verify:turbo-inputs` green; ignore-rule arm noted as CI's
...
- [ ] 8.0 Phase 5 — Testing: every §11 row, then the floor
  - [ ] 8.1 `node packages/provegate/dist/cli.js check --prompts` (FR-1)
  - [ ] 8.2 `pnpm verify:prompts` (FR-2)
  - [ ] 8.3 `pnpm verify:prompts-mutation` (FR-3; W5 restoration observed)
  - [ ] 8.4 `pnpm verify:workflow` + `gate check --wiring` (FR-4)
  - [ ] 8.5 `pnpm verify:doc-claims` (FR-5)
  - [ ] 8.6 `pnpm verify:turbo-inputs` (FR-6)
  - [ ] 8.7 `pnpm test --filter provegate` (FR-7, turbo-routed)
  - [ ] 8.8 `pnpm verify:review-quorum` + `pnpm verify:pack-drift` (FR-8)
  - [ ] 8.9 `pnpm verify:script-classes` + `gate check --wiring` (FR-9)
  - [ ] 8.10 Floor: `pnpm check-types` && `pnpm lint` && `pnpm test` &&
        `pnpm build`
  ...
  - [ ] 8.12 Re-read PRD §12 DO NOT — wrap-tolerant sweep.
- [ ] 9.0 Phase 6 — Final Auditing (W2)
  ...
  - [ ] 9.2 `pnpm verify:workflow` green after any fix; draft
...
| --------------------- | ------------------------------------------------------------------ | -------- | -------- | -------- | --------------------------------------------------------------------------------- |
| FR-1                  | `node packages/provegate/dist/cli.js check --prompts`              | repo     | pending  |          | all planned paths `current`                                                       |
| FR-2                  | `pnpm verify:prompts`                                              | repo     | pending  |          | committed store equals fresh render, via 034's script as shipped                  |
| FR-3                  | `pnpm verify:prompts-mutation`                                     | repo     | pending  |          | the bite: planted byte named `modified`, restored in `finally`, tree clean        |
| FR-4                  | `pnpm verify:workflow`                                             | repo     | pending  |          | the prompts check inside the bundle                                               |
| FR-4                  | `node packages/provegate/dist/cli.js check --wiring`               | repo     | pending  |          | registered and executing with real content                                        |
| FR-5                  | `pnpm verify:doc-claims`                                           | repo     | pending  |          | the entrypoint's new lines claim nothing that does not run                        |
| FR-6                  | `pnpm verify:turbo-inputs`                                         | repo     | pending  |          | no task hides the store from a gate that reads it                                 |
| FR-7                  | `pnpm test --filter provegate`                                     | repo     | pending  |          | listing-line test: phase-descriptive first line + parseable banner, every command |
| FR-8                  | `pnpm verify:review-quorum`                                        | repo     | pending  |          | semantic, both copies; loud on missing file                                       |
| FR-8                  | `pnpm verify:pack-drift`                                           | repo     | pending  |          | agreement guard on the root/packed pair                                           |
| FR-9                  | `pnpm verify:script-classes`                                       | repo     | pending  |          | both scripts classified; ADR rows bidirectional                                   |
| FR-9                  | `node packages/provegate/dist/cli.js check --wiring`               | repo     | pending  |          | CHECKS member + shrink-only exception; nothing wired nowhere                      |
...
| independent-review    | `_docs/reviews/review-032-prompt-store-dogfood.md` — `Critical: 0` | review   | pending  |          | W2 sweep briefed                                                                  |
