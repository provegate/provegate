...
- [ ] 0.0 Pre-flight
  ...
  - [ ] 0.2 Claim the lock lease for this work item with the PRD's Conflict Surface as
        `ownedPaths`; confirm no active claim overlaps.
  - [ ] 0.3 Branch `feat/prd-033-acceptance-authorship` from `main`.
  - [ ] 0.4 Record the pre-migration bytes of `_state/acceptances.json` (the committed
        blob at `HEAD`) as the baseline the mapping and the `method`-preservation
        assertions are checked against.

- [ ] 1.0 Data & Infrastructure — the field, the type, the migration (FR-1, FR-3)
  - [ ] 1.1 Add `authorship` to `packages/provegate/schemas/acceptances.schema.json`:
  ...
  - [ ] 1.2 Widen `AcceptanceEntry` in `packages/provegate/src/core/run/acceptance.ts`
        with `authorship: 'owner-written' | 'agent-transcribed'`. A union, never
        `string`, and never `any`.
  - [ ] 1.3 Migrate all sixteen entries in `_state/acceptances.json`. Mapping from the
...
- [ ] 2.0 Core Logic & Validation — enforcement and the close message (FR-2, FR-7)
  - [ ] 2.1 Add `authorship` to `ENTRY_FIELDS` in
        `packages/provegate/src/core/run/acceptance.ts`.
  - [ ] 2.2 Extend `entryProblem` to refuse a missing field and any value outside the
        enum, naming both legal values in the message. Refusal is reported by index,
        like every other entry problem.
  - [ ] 2.3 Leave `validAcceptance` unchanged. It must not learn about `authorship` —
        PRD §7 and §12, and `two-parsers-wrong-together`.
  - [ ] 2.4 Extend `OperatorGateResult`'s waived `why` in `operatorGateOk` to name the
        authorship beside the deciding owner.
  - [ ] 2.5 **Deny test** in `packages/provegate/test/acceptance.test.ts`:
  ...
  - [ ] 2.8 Test that a malformed entry fails the WHOLE store, not merely the selected
        entry — the existing comment at the validation loop says why.
  - [ ] 2.9 Test the waived reason on both branches: the `readCommitted` path and the
        `loadAcceptance` fallback.
  - [ ] 2.10 Re-read `two-parsers-wrong-together` before closing this parent; confirm
        exactly one function decides what a legal `authorship` is.

- [ ] 3.0 Governance Content Sweep — eight sites, two of them shipped (FR-4, FR-6, FR-8)
  - [ ] 3.1 Re-read `a-rule-corrected-survives-where-it-is-restated`, then re-run the
        ...
        Decisions**.
  - [ ] 3.2 `AGENT_BOOTSTRAP.md` — the **Operator acceptance** stop-and-ask row.
  - [ ] 3.3 `AGENT_BOOTSTRAP.md` — the critical-rules restatement near the end.
  - [ ] 3.4 `packages/provegate/METHOD.md` — the "Operator acceptance" section.
  - [ ] 3.5 `packages/provegate/practices/templates/AGENT_BOOTSTRAP.template.md` — the
        shipped stop-and-ask row. Same rule, same wording discipline.
  - [ ] 3.6 `packages/provegate/prompts/phase-6-final-auditing.md` — verify it points at
        METHOD.md rather than restating the rule; edit only if it restates.
  - [ ] 3.7 Amend `_brain/learnings/operator-acceptance-no-self-accept.md`: the rule
        survives, and transcription at explicit owner direction, recorded as such, is
        named as distinct from self-acceptance.
  - [ ] 3.8 Apply the identical amendment to
  ...
  - [ ] 3.9 Update both hooks: `_brain/INDEX.md` and
        `packages/provegate/practices/brain/INDEX.md`.
  - [ ] 3.10 Update both hashes for the record in `scripts/verify/pack-drift-ledger.json`.
  - [ ] 3.11 Write `_brain/adr/ADR-0003-acceptance-authorship.md`: the decision, the two
  ...
  - [ ] 3.12 Add the ADR pointer to `_brain/INDEX.md`.
  - [ ] 3.13 Case in `packages/provegate/test/content-canon.test.ts`: both SHIPPED sites
  ...
  - [ ] 3.14 Confirm `push-is-human-by-omission`, `gate push` and the runner are
...
- [ ] 4.0 Gate Wiring — one cache-free gate, registered everywhere (FR-5)
  - [ ] 4.1 Write `scripts/verify/verify-acceptance-rule.mjs`. Three assertions: the
  ...
  - [ ] 4.2 **W2** — record the residual in the gate's own header comment: the negative
  ...
  - [ ] 4.3 Register the `verify:acceptance-rule` key in `package.json`.
  - [ ] 4.4 Add the script to `CHECKS` in `scripts/verify/verify-workflow.mjs`.
  - [ ] 4.5 Add the manifest row in `gates.manifest.json`.
  - [ ] 4.6 Run `pnpm verify:gates-wired` — no on-disk check unregistered, no registered
        check unwired.
  - [ ] 4.7 Mutation check: revert one assertion at a time and confirm the gate fails on
        that assertion and only that one.

- [ ] 5.0 Migration & Rollback Plan
  - [ ] 5.1 Confirm the validator change (2.0) and the store migration (1.3) are staged
        into ONE commit. A repository between them cannot read its own acceptance store
        — PRD §7 and §12.
  - [ ] 5.2 **W1** — exercise the revert rather than asserting it: from the branch tip,
  ...
  - [ ] 5.3 Confirm the partial-rollback path is written in PRD §7 and matches what 5.2
        observed; correct the PRD if they disagree.

- [ ] 6.0 Phase 5 — Testing
  - [ ] 6.1 Run every command in PRD §11 and fill the Verification Ledger with evidence.
        No ad-hoc additions, no omissions.
  - [ ] 6.2 Run the cross-cutting floor: `pnpm check-types`, `pnpm lint`, `pnpm test`,
        `pnpm build`.
  - [ ] 6.3 Mutation check on each of the three deny tests: revert the enum check and
        confirm each fails, and that reverting it fails those tests and only those.
  - [ ] 6.4 Re-read PRD §12 and confirm no listed anti-pattern was introduced.

- [ ] 7.0 Phase 6 — Final Auditing
  - [ ] 7.1 Independent adversarial review; write
        ...
        including `Quorum`.
  - [ ] 7.2 Spec-vs-code audit: every FR against what landed, every Target against the
        merge diff.
  - [ ] 7.3 Re-run the site sweep independently of 3.1 — the reviewer sweeps, and does
        not merely confirm the author's list.
  - [ ] 7.4 Reproduce and fix every finding; record each in **Deferrals & Decisions**
        with its disposition.

- [ ] 8.0 Phase 7 — Learning
  - [ ] 8.1 Write `_brain/learnings/free-text-field-is-the-unread-drift-ledger.md` — a
  ...
  - [ ] 8.2 Add its pointer to `_brain/INDEX.md`.
  - [ ] 8.3 Durable-artifacts check: every path in the PRD's Durable Artifacts appears
        in the merge diff, and Memory Outputs and Durable Artifacts still agree.
  - [ ] 8.4 Close the `Acceptance authorship rule` deferral row in `STATUS.md`; if any
        part of it survives this PRD, retitle rather than delete it.
  - [ ] 8.5 Write `_docs/wip/summary-033-acceptance-authorship-provenance.md`.
  - [ ] 8.6 Capture protocol (`_brain/PROTOCOL.md` §7): anything non-derivable found
...
| ------------------ | ------------------------------------------------------------ | ----- | ------- | -------- | ----------------------------------------------------------- |
| FR-1               | `pnpm --filter provegate test test/acceptance.test.ts`       | pkg   | pending |          | schema required list + exactly two enum values            |
| FR-2               | `pnpm --filter provegate test test/acceptance.test.ts`       | pkg   | pending |          | missing and out-of-enum refused by index, both values named |
| FR-2               | `pnpm check-types`                                           | repo  | pending |          | authorship is a union of two literals, not a string       |
| FR-3               | `pnpm --filter provegate test test/acceptance.test.ts`       | pkg   | pending |          | mapping over the fixture; no method string changes        |
| FR-4               | `pnpm --filter provegate test test/content-canon.test.ts`    | pkg   | pending |          | the two shipped sites carry the new rule                  |
| FR-5               | `pnpm verify:acceptance-rule`                                | repo  | pending |          | cache-free: absence, four sites by path, store field      |
| FR-5               | `pnpm verify:gates-wired`                                    | repo  | pending |          | registered on every wiring surface                        |
| FR-6               | `pnpm verify:pack-drift`                                     | repo  | pending |          | record and twin reconcile, hash pair updated              |
| FR-6               | `pnpm verify:brain`                                          | repo  | pending |          | both INDEX hooks resolve; amended record parses           |
| FR-7               | `pnpm --filter provegate test test/acceptance.test.ts`       | pkg   | pending |          | waived reason names owner and authorship, both paths      |
| FR-8               | `pnpm verify:brain`                                          | repo  | pending |          | ADR-0003 parses with non-empty sections and is indexed    |
| types              | `pnpm check-types`                                           | repo  | pending |          | floor                                                     |
| lint               | `pnpm lint`                                                  | repo  | pending |          | floor                                                     |
| test               | `pnpm test`                                                  | repo  | pending |          | floor — added tests pass, existing unchanged              |
| build              | `pnpm build`                                                 | repo  | pending |          | floor                                                     |
| rollback (W1)      | revert the combined commit in a scratch worktree, validate   | repo  | pending |          | task 5.2 — evidence, not assertion                        |
| independent-review | `_docs/reviews/review-033-acceptance-authorship-provenance.md` | repo  | pending |          | verdict pass, critical = 0                                |
