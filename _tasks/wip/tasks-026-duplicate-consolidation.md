...
- [ ] 0.0 Pre-flight
  - [ ] 0.1 Open each Memory Context record; confirm named paths/commands still exist;
        stale findings → Deferrals & Decisions.
  - [ ] 0.2 W2: re-run `node packages/provegate/dist/cli.js queue`; PRD-028 shares
        `prd-ready.ts` — if it holds an execution-phase claim, serialize behind it.
  - [ ] 0.3 Confirm `_brain/adr/ADR-0004-method-rule-vs-repo-rule.md` is committed on the
        base branch and ratified (the owner's Phase-3 Go of 2026-07-28 is the
        ratification; it is recorded on the board). Absent → STOP.
  - [ ] 0.4 `gate open PRD-026 --worktree` from a clean base; lease on the MAIN checkout.
  - [ ] 0.5 Worktree: `pnpm install`; build `@provegate/design` then `provegate` (dist is
        gitignored); baseline `pnpm --filter provegate test` green; record the count.
  - [ ] 0.6 Re-measure the dated facts: root bundle ELEVEN members, packed SIX; CI steps
...
- [ ] 1.0 FR-1 — the review sweep (`gate check --review-artifacts`)
  - [ ] 1.1 `cli.ts::runCheck`: the flag beside `--wiring`; one line per invalid file;
        non-zero exit on any.
  - [ ] 1.2 Selection: files directly under the configured reviews dir matching
        `^review-.*\.md$` MINUS any basename containing `.template.`; not recursive.
  - [ ] 1.3 Binding: derive `expectedId` from `^review-(\d{3})-` under the configured id
        pattern and PASS it to the validator; a selected name yielding no id fails as
        unparseable, never skipped.
  - [ ] 1.4 Tests (`consolidation.test.ts`): valid-record-wrong-PRD fails; no-id name
        fails; `review-026-copy.template.md` NOT selected — each beside its passing
        control on the same shape.
- [ ] 2.0 FR-2 — the durable-artifacts declaration lint + sweep
  - [ ] 2.1 `durable.ts::declaredArtifacts`: adopt the script's asterisk exclusion; keep
        every-span collection; do NOT reintroduce any named-file predicate (the code
        comment already says why).
  - [ ] 2.2 `lintPrd`: the declaration rule — ≥1 bullet, each bullet a path-bearing claim
        or explicit `none`; mixing legal; absent/empty/neither fails.
  - [ ] 2.3 `cli.ts::runCheck`: `--durable-artifacts` sweep over configured wip PRDs.
  - [ ] 2.4 Corpus pass BEFORE the lint lands (strictly stricter): run across every wip
        PRD; a newly failing section is reported to its author, never edited to fit.
  - [ ] 2.5 Tests: mixed section passes; absent/empty/neither-bullet fail; asterisk
        excluded; two-path bullet is two claims.
- [ ] 3.0 FR-8 — the class ledger (BEFORE the deletion commit, so the bundle gains its
      ninth member in the same change that drops three)
  - [ ] 3.1 `scripts/verify/script-classes.json`: eleven entries exactly matching
        ADR-0004's table; `verify-brain`, `verify-deferred`,
        `verify-memory-record-corpus` as `method-pending` (owner, reviewBy 2026-10-01);
        the rest `repo`.
  - [ ] 3.2 `scripts/verify/verify-script-classes.mjs` via the shared `targetRoot()`:
  ...
  - [ ] 3.3 Register `verify:script-classes` in `package.json`; membership in the root
        bundle `CHECKS` (its wiring surface).
  - [ ] 3.4 Tests: mutate-one-green-baseline per fixture — unclassified, stale, expired,
        malformed-pending, table-vs-ledger disagreement — each asserting its SPECIFIC
        issue beside the passing baseline pair.
- [ ] 4.0 FR-4 — manifest and CI run the CLI (retain-plus-add)
  - [ ] 4.1 `package.json`: `check:review-artifacts` and `check:durable-artifacts`
        aliases (`node packages/provegate/dist/cli.js check --…`) — non-verify names, by
        design.
  - [ ] 4.2 `gates.manifest.json` phase 4: today's six commands + the two aliases, build
        preceding them.
  - [ ] 4.3 `ci.yml`: REPLACE the three alias steps (`:75-88`) with the sweep invocations
        after the build step; removal and addition in one edit.
  - [ ] 4.4 Test: every manifest command resolves to an existing `package.json` script;
        no §11 row nor manifest entry names a deleted script.
- [ ] 5.0 FR-6 — the documentation sweep (before the deletion commit; the docs stop
      naming what is about to go)
  - [ ] 5.1 Rewrite the six live documents to the surviving CLI surface;
        `NEXT_STEPS.md` additionally carries fresh-install guidance + migration pointer.
  - [ ] 5.2 The boundary check in `consolidation.test.ts`: walk every tracked `*.md`
        ...
        names.
  - [ ] 5.3 Vacuity control: a planted live-shaped mention inside the scan set is
        refused; the STATUS.md exclusion does not swallow the board's live sections.
- [ ] 6.0 FR-3 + FR-5 — THE DELETION COMMIT (one commit, both halves)
  - [ ] 6.1 Repo: delete the three scripts + `gates-wired-exceptions.json`; remove the
        three `package.json` entries; root bundle `CHECKS` 11 → 9 (trio out, ledger in).
  - [ ] 6.2 Pack: delete the three packed twins + packed exceptions file; packed bundle
        `CHECKS` 6 → 3.
  - [ ] 6.3 `init.ts::PACK_MAP`: remove the four entries (the drift check's single source
        of pairing).
  - [ ] 6.4 `pack-drift-ledger.json`: remove the four pairs (three hash pairs + the
        pack-tracked exceptions entry with its note); reconcile.
  - [ ] 6.5 `pack-manifest.json` + `pack.test.ts`: the shipped-file allowlist shrinks by
        four.
  - [ ] 6.6 Upgrade fixture (`init.test.ts`, production `initWorkspace` pair over an
  ...
  - [ ] 6.7 Conversion fixture (`practices-pack.test.ts`): the retained eight-entry
...
- [ ] 7.0 FR-7 — the release
  - [ ] 7.1 `.changeset/` minor entry carrying ALL FIVE migration steps including the
        exceptions conversion; compatibility sentence present.
  - [ ] 7.2 `changeset-entry.test.ts`: this entry's own assertion group discriminated by
        its own tokens (the two sweep flags); the distinct-files test covers all three
        qualifying entries.
- [ ] 8.0 Migration & Rollback Plan (infra-class explicit parent)
  - [ ] 8.1 Prove the both-halves revert: reverting the deletion commit + the FR-4 edit
  ...
  - [ ] 8.2 Assert the adopter no-op property as documented: neither migrated nor
        unmigrated adopters are touched by a rollback (prose + the additive-only fixture
        already proves the mechanism).
  - [ ] 8.3 Confirm the ledger reverts WITH the consolidation (no impossible
        method/method-pending state can be constructed post-revert).
- [ ] 9.0 Phase 5 — Testing (every §11 row, no additions, no omissions)
  - [ ] 9.1 The twelve §11 rows exactly as written (five consolidation.test.ts rows, the
        two repo sweeps, init/pack/practices-pack/changeset rows, verify:script-classes
        live row).
  - [ ] 9.2 Floor: `pnpm check-types`, `pnpm lint`, `pnpm test`, `pnpm build`,
        `pnpm verify:workflow` (nine-member bundle green).
  - [ ] 9.3 Mutation checks: revert the asterisk exclusion, the template exclusion, and
        one ledger schema rule in isolation — each kills exactly its paired fixture.
  - [ ] 9.4 Update the Verification Ledger with evidence per row.
...
| ------------------ | ----------------------------------------------------------- | ----- | ------- | -------- | ----- |
| FR-1               | `pnpm --filter provegate test test/consolidation.test.ts`   | pkg   | pending |          | review sweep: select-then-bind, wrong-PRD fails |
| FR-2               | `pnpm --filter provegate test test/consolidation.test.ts`   | pkg   | pending |          | declaration lint + reconciled parser |
| FR-3               | `pnpm verify:workflow`                                      | repo  | pending |          | nine-member bundle green |
| FR-4               | `pnpm --filter provegate test test/consolidation.test.ts`   | pkg   | pending |          | every manifest command resolves |
| FR-5               | `pnpm verify:pack-drift`                                    | repo  | pending |          | pairs removed both sides |
| FR-5               | `pnpm --filter provegate test test/init.test.ts`            | pkg   | pending |          | upgrade fixture through the executor |
| FR-5               | `pnpm --filter provegate test test/pack.test.ts`            | pkg   | pending |          | allowlist shrank with the pack |
| FR-5               | `pnpm --filter provegate test test/practices-pack.test.ts`  | pkg   | pending |          | packed bundle three; conversion audit-accepted |
| FR-6               | `pnpm --filter provegate test test/consolidation.test.ts`   | pkg   | pending |          | boundary + vacuity control |
| FR-7               | `pnpm --filter provegate test test/changeset-entry.test.ts` | pkg   | pending |          | five steps; discriminated selector |
| FR-8               | `pnpm --filter provegate test test/consolidation.test.ts`   | pkg   | pending |          | mutate-one-baseline fixtures via targetRoot |
| FR-8               | `pnpm verify:script-classes`                                | repo  | pending |          | live ledger+record pair; trio in neither store |
| types              | `pnpm check-types`                                          | repo  | pending |          |       |
| lint               | `pnpm lint`                                                 | repo  | pending |          |       |
| test               | `pnpm test`                                                 | repo  | pending |          | existing tests unchanged |
| build              | `pnpm build`                                                | repo  | pending |          |       |
| bundle             | `pnpm verify:workflow`                                      | repo  | pending |          | the remaining bundle green |
