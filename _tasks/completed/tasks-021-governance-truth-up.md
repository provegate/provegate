...
| ---- | --------- | ----- | -------------- | ------ | ----- |
| 12.5 | manual-qa | owner | Accept that editing the root `workflow.config.json` advances the control-artifact base, so any lease taken before it is refused until that worktree merges | accepted | Owner-directed in chat 2026-07-27; recorded in `_state/acceptances.json` with the method stated. Measured at close: one lease, this PRD's own. The agent lists the affected leases at merge time; deciding to land a change that stops in-flight work is an owner call |
| 15.4 | manual-qa | owner | Sign the close acceptance — this PRD changes the **published config surface**, so Autonomous Close is operator-gated | accepted | Owner-directed in chat 2026-07-27 after two independent rounds and fourteen remediated findings. Recorded in `_state/acceptances.json`; the `method` field states it was agent-recorded on owner instruction, not an interactive confirmation |
