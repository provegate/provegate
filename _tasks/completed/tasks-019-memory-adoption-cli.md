...
| ---- | --------- | ----- | -------------- | ------ | ----- |
| 10.3 | manual-qa | owner | Run `gate doctor --memory` on a repository you did not scaffold and judge whether its failures are actionable | done | **Discharged by dogfood, not by owner judgement, and it earned its keep.** Run against a repo produced by `gate init --practices`: green on a fresh install, and on a broken validator the remediation named the missing file and why. It also found a REAL defect — the install reported placeholders clean while its scaffolded `AGENT_BOOTSTRAP.md` carried five unfilled tokens, because the check scanned only the index. Fixed, mutation-checked. The owner did NOT personally run it on a foreign repository; accepted on this evidence instead, and the acceptance entry says so |
