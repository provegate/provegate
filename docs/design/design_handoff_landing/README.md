...
## Repo delta — what already exists vs. what is new
`apps/web` already implements an earlier cut of this page. Its `app/page.tsx` renders:
`Nav · Hero · Problem · CoreRule · Method · RunWalkthrough · Refusal · EvidenceLedger · Proof · Positioning · CommandRef · Footer`,
with section source in `apps/web/app/sections/` (`index.tsx`, `content.ts`, `reveal.tsx`) and tests in `apps/web/test/`.

**The task is to bring `apps/web` up to this prototype**, not to start over:
- **Already there (verify against the prototype, adjust copy/layout only):** Nav, Hero, Problem, CoreRule, Method (= Phases), RunWalkthrough (= How / `gate run`), Refusal, EvidenceLedger (= Ledger), Proof, Positioning, CommandRef, Footer.
- **New sections to add, in this order:** `TrustStrip` (after Hero) · `Playground` (after How) · `PhaseDetail` + `OperatorFlow` (after Phases) · `Anatomy` (after Proof) · `Comparison` (before Positioning) · `Features` · `InstallTabs` · `CIIntegration` · `FaqAndQuickstart` · `Install` (final CTA, before Footer).
- Keep new copy in `app/sections/content.ts` and extend `content-web.test.ts` / `a11y.test.ts` / `landing.test.tsx` to cover the added sections.
- Sections named in `landing-extras.jsx` map 1:1 to the list above (`PG_Playground` → `Playground`, etc.).

## Do NOT port the Tweaks panel
`landing-app.jsx` carries prototype-only exploration controls (`depth`, `rhythm`, `voice`) plus the `PG_RHYTHM_CSS` / `PG_VOICE_CSS` override blocks and the `TweaksPanel` import. These exist so the design could be compared in-canvas. **Ship the chosen setting only:**
- depth **full** (every section renders — `PG_ORDER`),
- rhythm **standard** (no spacing overrides),
- voice **editorial** (no type overrides).

So: ignore `TWEAK_DEFAULTS`, `PG_ESSENTIALS`, `PG_SKIM`, both CSS override maps, the `data-rhythm` / `data-voice` attributes, `tweaks-panel.jsx`, and the `show()` gating — render `PG_ORDER` unconditionally. The dark/light `data-theme` toggle in the Nav **is** real product behavior and should be kept.
...
- `tweaks-panel.jsx` — prototype tweak-panel host. **Do not port** (see above).
