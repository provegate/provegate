# `_brain` — Agent-Agnostic Memory Protocol (provegate)

Wave-1 deliverable. Implementable spec. A provegate coding agent can build `_brain` from
this document alone.
...
Note on import: the handoff ships the templates under
`01-brain-memory-protocol/templates/`; they land at `_brain/_templates/` (rename on copy).
...
scope: workflow                           # workflow | project   (the de-emofy knob)
...
## 12. Acceptance (definition of done for wave 1)

- [ ] `_brain/` exists with the §3 layout.
- [ ] `INDEX.md` present, grouped, pointing at every seeded record — no orphans.
- [ ] `_templates/learning.md` + `_templates/adr.md` present and match §4 / §11 (imported
      from the handoff's `templates/` dir — note the rename to `_templates/`).
- [ ] This SPEC imported verbatim as `_brain/PROTOCOL.md` — the canonical protocol must
      survive the deletion of `_handoff/`.
...
- [ ] `_handoff/` scaffolding deleted from provegate after import.
