...
When each work-item runs in its own git worktree and its claim/lock is written *inside that
worktree*, removing the worktree at release (or crash) deletes the lock too — but other
...
Release deletes the lock explicitly. TTL expiry unblocks *rivals* (the overlap engine
