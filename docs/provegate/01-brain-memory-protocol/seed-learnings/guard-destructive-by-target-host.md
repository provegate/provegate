...
drop against prod. Guarding by the *target connection host* is robust to that class of
misconfiguration.
...
allowed; production (by host, or an explicit target-env token) refused *unconditionally*;
