...
4. **Write the lock (§3)** to the **main checkout's** lock dir, not the worktree (seed
...
5. **Hydrate** — symlink the root `.env`/`.env.local` by existence only (never read values;
   seed `fresh-worktree-env-gap`), install deps.

Terminology guard — three different "protected" sets exist; don't build one constant:
- the **commit-guard set** (practice 02): `{main, master, staging}` — merge-only for source;
...
See `templates/agent-lock.schema.json`. Fields:
...
"second block" practice 02 drops for single-agent projects — re-adopt it here.)
...
- **Pre-start check** runs **before** any worktree/lock is created, and builds the candidate from
  ...
  override*, so a caller can't dodge the check by mirroring a false surface (seed
...
set is a *superset* of the union list. Keep both lists as project config. (seed
`append-only-manifest-union-driver`)
...
- **What CI can check is the board:** `status-sync --ci` drops lock-matching and enforces
  **"promotion PRs carry no active claims"** — a PR into a promotion target (the
  ...
  stale > 7 days.
- A local **pre-commit backstop** blocks staging source on a base branch (practice 02), and
...
shared-append-only file list (project config). Emofy leaves the JSON Schema doc-only (real gate
