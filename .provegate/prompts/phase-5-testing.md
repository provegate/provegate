...
2. **Real environment.** Integration commands run against the real thing
   (Hermetic — tests run against temp-dir git fixtures; no network, no external services, no `.env` required.), not mocks. If the environment is unavailable, the ledger row is
...
advance. `gate run PRD-XXX` executes exactly this chain mechanically — a
...
> "Phase 5 complete for PRD-XXX. Every §11 command executed (N passed /
