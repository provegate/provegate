...
<!-- No push code path anywhere (CLI or CI; only the human-triggered release workflow publishes). `packages/provegate`: zero runtime dependencies, no telemetry, no network calls. Method content traceable to the source snapshot. ADRs bind until superseded. Canonical statusVocab values only.: your always-verify list plus per-category blocks (data,
