...
<!-- TypeScript strict, ESM; pnpm + turbo monorepo; zero runtime deps in `packages/provegate`; Conventional Commits with lower-case subjects; design primitives from `@provegate/design` (web consumes the built dist).: add your stack-specific sections here (schema changes, API
...
or `- Deferred to [PRD-NNN](<path>)` where `NNN` runs at the configured id
...
- learning: `_brain/learnings/[slug].md` — [the durable fact expected]
...
- `_brain/[page].md` — [what is learned] | `none`
- ADR: `_brain/decisions/adr-XXX-[slug].md` | `none`
- `_brain/learnings/[slug].md` — every Memory Output above repeats here; the two
...
| ---- | --------------------- | ----- | ----- |
| FR-1 | `pnpm test --filter provegate` |       |       |
| FR-2 | `pnpm test --filter provegate` |       |       |
...
Before Phase 2 PASS, run: `gate check PRD-XXX`
...
<!-- No push code path anywhere (CLI or CI; only the human-triggered release workflow publishes). `packages/provegate`: zero runtime dependencies, no telemetry, no network calls. Method content traceable to the source snapshot. ADRs bind until superseded. Canonical statusVocab values only.: add your project-wide forbidden moves (scoping bypasses,
