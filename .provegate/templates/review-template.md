<!-- GENERATED by provegate 0.3.0 — `gate init --prompts`. Do not edit.
     This store installs ONE WAY: to reinstall, delete every path the command
     printed and run it again. -->

# Independent Review: PRD-XXX — [Feature Name]

> Save to your reviews directory (default `_docs/reviews/review-XXX-{short-name}.md`).
> The metadata block below is machine-validated by `gate run`'s Phase 6 gate:
> all six fields required; `pass` requires `Critical: 0` (strictly numeric).
> Quorum records the reviewer panel outcome — the method's gate is >=3/5 pass
> (see prompts/orchestration-runner.md); record the actual N/M your panel produced.

> **PRD:** PRD-XXX
> **Verdict:** pass | fail
> **Reviewer:** [tool/model — must not be the implementing agent]
> **Base SHA:** `[git merge-base or base tip]`
> **Critical:** 0
> **High:** 0
> **Medium:** 0
> **Quorum:** 3/5 pass

## Summary

One paragraph: what was reviewed (diff range, review rounds), overall risk.

## Findings

| #   | Sev      | Finding | Resolution   |
| --- | -------- | ------- | ------------ |
| 1   | CRITICAL | …       | fixed/waived |

## Post-fix verification

Commands re-run after fixes (if any), with results.
