...
feature branch (`feat/prd-XXX-<slug>`) and a linked worktree under the configured
`worktree.dir` are provisioned, and the lease is stamped with both. Provisioning
failure (branch or path collision) rolls the lease back — claim and checkout are one
outcome. After `gate run` lands the merge, the runner removes the worktree and
deletes the merged branch; a dirty worktree refuses removal and degrades to a
handoff-card warning (never `--force`, never `-D`). Locks stay platform-agnostic:
external worktree managers keep working, the flag is optional sugar.
