...
default), writes the snapshot (default `_state/prds.json`), and prints an aligned
table — one line per work item — plus panel metrics:

```
$ gate status
ID       STATE        READINESS   TASKS  SLUG
PRD-001  In Progress  PASS · 8.7  12/45  config-state-locks
Implemented: 0
Latest implemented: —
