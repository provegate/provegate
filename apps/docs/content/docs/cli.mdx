...
The checkout is cut from the configured base ref, so the PRD (and
`workflow.config.json`, when present) must be committed there first — an
uncommitted PRD refuses the claim instead of producing a checkout missing the
artifact it claims. Provisioning failure (branch or path collision) rolls the
