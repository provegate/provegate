...
The one place repo shape or toolchain differs is the `commands` block in
`workflow.config.json` — point it at your own checks:
...
The commands are plain strings, so no pnpm or turbo is assumed — a direct toolchain
(`tsc --noEmit`, `eslint .`, `vitest run`, …) works just as well. `gate check
--wiring` confirms they resolve.
