...
One thing the pack does **not** do: wire itself. It creates files and then prints the
manual steps — the `package.json` scripts to add, the CI job to run them in, and the
agent-entry shims to paste. That list is `NEXT_STEPS.md`, printed at the end of the
install and kept in the package. Editing your `package.json` and your CI config is not
something a scaffold should do without you.
...
`gate init` writes an **empty** `gates.manifest.json` (no gates until you wire
your own). Add your commands there — `phases."4"` is the floor the run executes,
`postMerge` runs after the local merge:
