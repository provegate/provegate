...
function useTyping(steps: readonly string[], active: boolean): { line: number; typed: string; finished: boolean } {
          ...
          return (
            <div key={step}>
              <div>
                ...
                {current ? (
                  <span className="pg-caret" style={{ color: 'var(--pg-term-fg)' }} aria-hidden="true">
                    █
                  </span>
                ) : null}
              </div>
        ...
        {done ? (
          <div style={{ marginTop: 6 }}>
            {C.HERO_TERMINAL.gates.map((g) => (
              <GateLine key={g.command} bare status="passed" name={g.name} command={g.command} code={g.code} />
            ))}
            <div style={{ marginTop: 8, color: 'var(--pg-term-green)' }}>{C.HERO_TERMINAL.earned}</div>
