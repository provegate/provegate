...
7. **FR-7**: The generated Claude commands list usefully. Claude Code reads a command
   ...
   - **Targets:** `packages/provegate/src/core/run/prompts.ts`,
     ...
     matcher confirms it covers `prompts.test.ts`). This file is one of FOUR measured
     036 overlaps, and the sequencing rule is FR-9's, total: any one overlap holds the
     ENTIRE PRD until 036 releases its lease — re-run `gate queue` at Phase 3.
...
- **PRD-036's lease released** — four measured overlaps exist and `gate open` reads
  the whole declared surface, so the ENTIRE PRD holds until then (FR-9's rule; no
  partial start exists).
...
- [ ] FR-9 integration: the `verify-workflow.mjs` CHECKS member line, the
      ...
      both ADR-0004 table rows — and the WHOLE PRD holds until 036 releases its lease
      (four measured overlaps; `gate open` reads the entire surface, no partial start)
...
- `packages/provegate/test/prompts.test.ts` — **contested with PRD-036's
  ...
  claim `06efc01`). One of FOUR measured 036 overlaps on this surface (with
  `verify-workflow.mjs`, `script-classes.json`, ADR-0004 below) — and `gate open`
  evaluates the whole declared surface, so any one of them holds the ENTIRE PRD until
  036 releases; re-run `gate queue` at Phase 3
...
- `scripts/verify/verify-workflow.mjs` — **contested: inside PRD-036's live Phase-4
  lease; FR-9's CHECKS append lands only after 036 releases (whole-PRD hold, FR-9's
  total rule)**
...
- DO NOT start ANY implementation while PRD-036's lease is active. Four overlaps are
  measured, `gate open` evaluates the whole declared surface, and no partial claim of
  the unleased subset exists — the entire PRD waits for the release, then
  `gate queue` → `gate open`.
...
| Date       | Author | Changes                                                                                                                                                                                                                                              |
| ---------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 2026-07-29 | orchestrating session (mechanical, per iteration-6 finding) | **Iteration-6 remediation — the last two "one file" survivors.** The whole-PRD serialization rule landed at iteration 5 in FR-9 and Memory Inputs, but "serializes on this one file" survived in FR-7's Targets note and the Conflict Surface — the ninth restatement instance, and factually false: the live `candidateConflicts` probe returns FOUR shared paths with 036. Both sites now carry the total rule, and it is reinforced explicitly where the scorer asked: §7 Dependencies (a new lease-released prerequisite bullet), §8's FR-9 bullet, and a DO NOT line. Post-edit sweep run as a separate pass (grep families: "one file", "disjoint edits", partial/freely, serialization wording) — recorded here, in the row after the corrections. |
