...
> **Status**: Superseded
>
> **Superseded by**: PRD-024 (`readiness-lint-parsers`), PRD-025
> (`wiring-audit-completion`), PRD-026 (`duplicate-consolidation`) — owner decision
> 2026-07-27. Six independent readiness rounds scored this PRD between 6.65 and 7.19
> against a PASS threshold of 8.0, with no trend: each remediation closed real findings and
> introduced roughly as many. Iteration 6 located the cause in size rather than care — two
> live defects had hidden through five adversarial reviews — and its own findings, plus the
> six documents FR-6 of PRD-026 now covers, are distributed across the three successors.
> **Nothing found here is discarded.** The thesis, the measurements, and every round's
...
| 2026-07-27 | Claude Opus 5, on owner direction | **Superseded by PRD-024, PRD-025 and PRD-026 (owner decision after iteration 6).** Four independent rounds — 6.65, 7.19, 6.85, 6.92 — inside a half-point band, against a threshold of 8.0. Iteration 6 diagnosed size rather than care: three of its five [P1]s were introduced by the preceding remediation, and **two were pre-existing defects that five adversarial rounds had not found** (FR-5's only verification command was a script FR-4 deletes; three shipped documents still instructed readers to run the removed checks). A document where that can happen is too large to review. The split follows the seams where every round's findings clustered: **PRD-024** takes FR-7's three lint-parser defects plus the corpus-command defect found in it, and has no dependency on the others; **PRD-025** takes FR-1, FR-4 and FR-6 — the wiring audit and the class ledger — and must land before any deletion; **PRD-026** takes FR-2, FR-3, FR-5, FR-8 and FR-9, holding every deletion in one commit because pack-drift pairs the two sides. Findings T, V, W and X are resolved in the successors rather than here: FR-4's verification no longer names a deleted script, the mutation check no longer fails on a missing pack file, the migration is five steps everywhere, the interpreter grammar is closed, and the documentation sweep grew from the three documents the review named to the **six** a re-measurement found — three of them shipped, one being the registration instructions that created the duplication at every adopter. This document stays as the record of how the problem was understood |
