...
9. **FR-9**: `CodeBlock`'s `copyable` affordance copies. Today it renders
   ...
   **Tests, both layers.** Design (`props.test.tsx`): the button renders with the
   ...
   - **Targets:** `packages/design/src/react/CodeBlock.tsx`,
     ...
     `packages/design/README.md`, `apps/web/README.md`,
...
**FR-1 and FR-8 are one atomic rollback unit (iteration-7 precision).** FR-8 exists
*because* FR-1 makes the root card reach `/alt`. Reverting FR-8 alone leaves the root card
in place with no override on `/alt`, restoring exactly the impersonating unfurl FR-8 was
written to prevent, on a page that is once again indexable. The rule: **revert both in one
commit.** A sequential order exists only as a non-deployable working-tree state — FR-8's
revert may precede FR-1's in the tree, but nothing between the two may build or deploy —
and the atomic joint revert is the only sequence this PRD endorses. Never FR-8 alone.
...
- [ ] `packages/design/README.md` + `apps/web/README.md` — import-contract descriptions
      follow the split renderer (FR-9); `apps/docs/README.md` deliberately unchanged
