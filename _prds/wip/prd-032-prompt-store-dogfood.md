...
1. **FR-1**: `workflow.config.json` gains the `prompts` block for this repository: `dir`
   ...
   No `<PROVEGATE:UNSET>` sentinel survives, and the render refusing otherwise is what makes a
   green result proof that every value was answered rather than defaulted.
...
- DO NOT leave a `<PROVEGATE:UNSET>` sentinel in `workflow.config.json` to make the render
  pass. The refusal is the proof that every value was answered.
