...
2026-09-26, closed by this FR). FR-8 removes the second review-template authority (the
root template calls Quorum optional while the shipped template and the review gate refuse
its omission). Both are adopter-facing package/content closures; without them the dogfood
...
| Review-template authorities in this repository                      | 2       | 1      | the root template carries the `quorum-is-required` marker   |
...
3. **FR-3**: The existing check goes live, unmodified — and its bite is proven by an
   **executed** mutation probe, not prose. PRD-034's `scripts/verify/verify-prompts.mjs`
   ...
   in a `finally`, and asserts the tree is clean again. Deliberately a §11-executed
   probe and NOT a `verify:workflow` CHECKS member: it is Phase-5 evidence that the gate
   bites, not a standing invariant, and the CHECKS bundle stays PRD-034's as shipped.
...
- **Building or modifying the reconciliation check.** PRD-034 owns the primitive, the
  ...
  nothing itself.
- **Changing method content.** Prompt/template prose is PRD-031's territory and the
  source snapshot's law. FR-7 changes emitted adapter FRAMING (code in `prompts.ts`),
  not a protocol's prose; FR-8 edits a repository-local document, not a shipped one.
  This PRD renders whatever the package ships at the time it runs.
...
- **Given** `_docs/review-artifact.template.md`, **When** a reader checks the Quorum
  rule, **Then** it matches the review gate's real refusal behavior and carries the
  `quorum-is-required` marker — one authority, not two.
...
is inside the atomic step. Deployment ordering beyond this repository: none — nothing
here ships in the package except FR-7's adapter change, which is version-gated by the
normal release flow.
...
- DO NOT start before the aggregate is green. PRD-029 and PRD-034 are both Ship Verified
  ...
  main, and the red doc-claims figure is not this PRD's to fix.
