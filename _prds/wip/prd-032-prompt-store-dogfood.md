...
| ----------------------------------------------------------------- | ------- | ------ | ---------------------------------------------------------- |
| Phase protocols reachable by an agent working in this repository   | 0       | 12     | store inventory at the repository root                     |
| Agent-config files here pointing at a phase protocol               | 0       | 3      | generated adapters, committed                              |
| Hand-written bytes in the store or the adapters                    | n/a     | 0      | committed render equals a fresh render, checked cache-free  |
| Placeholder values this repository leaves unfilled                 | 13      | 0      | the render refuses otherwise, so a green check is the proof |
...
- [ ] `.cursor/rules/prd-workflow.mdc` attaches on the four artifact directories, and
      `.claude/commands/prd-*.md` exists per phase.
...
- [ ] The check fails when a store file is edited by hand without a ledger exception.
...
1. **FR-1**: `workflow.config.json` gains the `prompts` block for this repository: `dir`
   (`.provegate`), `adapters` (all three), and `values` filled for all **thirteen**
   non-config tokens, each with this repository's real answer — `{{ARCHITECTURE_DOC}}` is
   `AGENT_BOOTSTRAP.md`, `{{MEMORY_ROOT}}` already resolves from `memory.root`,
   `{{REVIEW_TOOL}}` is the `/codex` invocation this repository actually uses, and so on. No
   sentinel survives; the render refuses otherwise, which makes a green result the proof
...
3. **FR-3**: A `scripts/verify/` check asserts the committed store and adapters equal a
   ...
   ledger and exception rules to PRD-030's primitive rather than reimplementing them. It
...
(PRD-030). It does not need PRD-031: whatever method content the package ships at the time
this runs is what renders, and PRD-031 landing later simply changes the rendered bytes,
which FR-3's check will then require to be re-committed. That is the mechanism working, not
a conflict.
...
- [ ] `workflow.config.json` — the `prompts` block with all thirteen values answered
...
- reviewed: `evidence-pattern-satisfied-by-the-template` — FR-1's proof that every value was
  ...
  because "the config has thirteen keys" is exactly the satisfied-by-the-template shape.
...
| ---- | ---------------------------------------------------- | ----- | ----------------------------------------------------------------------------------------------------------- |
| FR-1 | `node packages/provegate/dist/cli.js doctor --prompts` | repo  | every path reports match, which is only reachable when all thirteen values are answered                     |
...
- DO NOT reimplement the ledger or exception rules here. The check delegates to PRD-030's
...
- DO NOT leave a sentinel value in `workflow.config.json` to make the render pass. The
  refusal is the proof that every value was answered.
