...
- learning: `_brain/learnings/lint-must-name-the-span-it-judges.md` — that a section-scoped
  ...
  noticed.
- learning: `_brain/learnings/runner-sentinel-blocks-cli-spawning-tests.md` — appended
  during implementation (the memory contract allows a discovered output with a rationale):
  the first `gate run` of this PRD stopped at Phase 5 because a §11 row invoking the whole
  package suite reaches `revalidate.test.ts`, whose fixtures spawn the CLI in isolated temp
  repos — and the runner's `PROVEGATE_RUN_ACTIVE` re-entry sentinel propagates through the
  environment into those children and refuses them. Not derivable from any single file:
  the sentinel, the test, and the §11 row are each correct alone.
...
- Learning: `_brain/learnings/runner-sentinel-blocks-cli-spawning-tests.md` — the
  appended Memory Output above, repeated here because the two lists are one contract
...
| FR-2 | `pnpm test`                                                 | repo  | the whole suite stays green, via turbo — the direct package invocation cannot run under the runner: revalidate fixtures spawn the CLI and the re-entry sentinel refuses them; turbo's cache key is honest after this PRD's own inputs work, so a replay is a valid green |
...
| Date       | Author | Changes       |
| ---------- | ------ | ------------- |
| 2026-07-28 | Claude Fable 5 (implementing session), during Phase 4 | **Phase-4 discovery, recorded as the memory contract requires: the whole-suite §11 row cannot run under the runner.** The first `gate run PRD-024` stopped at Phase 5: `pnpm --filter provegate test` reaches `revalidate.test.ts`, whose fixtures spawn the gate CLI in isolated temp repos, and the runner's `PROVEGATE_RUN_ACTIVE` re-entry sentinel rides env inheritance into those children and refuses them — ten failures in a file green under plain `pnpm test`. The row now goes through turbo (`pnpm test`), which is a valid form precisely because of this PRD's own FR-2: the test task's inputs are honest, so a cache replay is a real green rather than a mask. Captured as an appended Memory Output (`runner-sentinel-blocks-cli-spawning-tests`) with its Durable Artifacts twin; the independent review also added one input during its round: the corpus test's root config reads (`workflow.config.json`, `gates.manifest.json`) joined the turbo inputs after Codex's round-1 [P1], re-verified clean in round 2 |
