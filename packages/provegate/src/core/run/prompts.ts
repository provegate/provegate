...
export function reconcilePrompts(
  ...
  for (const [joined, expected] of fresh) {
    ...
    const banner = BANNER_VERSION_RE.exec(onDisk.toString('utf8'));
    const bannerVersion = banner?.[1] ?? null;
    findings.push({
      path: canonical,
      kind: bannerVersion === null ? 'unattributable' : bannerVersion === version ? 'modified' : 'stale',
      bannerVersion,
