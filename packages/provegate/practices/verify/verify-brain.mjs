...
/**
 * The human hook after the Markdown link, in characters. The index is read on
 * every task, so its cost is paid constantly and its budget is small. Link markup
 * is excluded — the budget is for the sentence a reader skims, not for the path
 * length that happens to sit in front of it.
 */
const HOOK_MAX = 120;
